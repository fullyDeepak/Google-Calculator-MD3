...
function checkValue(key) {
    ...
    value = (key >= '0' && key <= '9') || ['+','-','%','.','(',')','ArrowLeft','ArrowRight','Delete','Control','Backspace'].includes(key)
    return value;
  }
...
for (let i =0; i<calcButtons.length; i++){
    if (i==18){
        document.querySelectorAll(".calc-button")[18].addEventListener("click", delLast)
    }
    else{
        document.querySelectorAll(".calc-button")[i].addEventListener("click", printValue);
    }
    // console.log(i)
}
